...
void drawSprites(Shader * shad, glm::vec3 p1, glm::vec3 radius, glm::vec3 colour, unsigned int tex, bool usetex, int r, int c, int frame)
{
	shad->use();
	...
	glm::vec2 c1(0.0f, 1.0f);
	glm::vec2 c2(0.0f, 0.0f);
	glm::vec2 c3(1.0f, 0.0f);
	glm::vec2 c4(1.0f, 1.0f);

	//y = trunc(id / w);
	//x = id - w * y;

	int rowid = trunc(frame / c);
	int colid = frame - c * rowid;

	glm::vec2 csize(1.0f / (float)c);
	glm::vec2 cshift(colid, rowid);
	
	c1 = (c1 / glm::vec2(c))+cshift*csize;
	c2 = (c2 / glm::vec2(c))+cshift*csize;
	c3 = (c3 / glm::vec2(c))+cshift*csize;
	c4 = (c4 / glm::vec2(c))+cshift*csize;

	//printf("%d: %d  %d (%f   %f)\n", frame, colid, rowid,c1.x,c1.y);
	...
	verts.push_back(c1.x);
	verts.push_back(c1.y);
	...
	verts.push_back(c2.x);
	verts.push_back(c2.y);
	...
	verts.push_back(c3.x);
	verts.push_back(c3.y);
	...
	verts.push_back(c4.x);
	verts.push_back(c4.y);
